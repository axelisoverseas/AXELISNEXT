 ...
 <section className="relative sec overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-6xl font-extrabold text-white tracking-tight mb-6">
            Independently certified by <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-[var(--accent-on-dark)]">DPIIT, British Council and AIRC</span>
          </h1>
            ...
            {certifications.map((cert) => (
              ...
              >
                ...
                <div className="relative glass-storm p-6 h-full flex flex-col items-center text-center">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 border border-[var(--color-rule)] bg-gradient-to-br ${cert.accent} text-[var(--color-navy)]`}>
                    <cert.Icon size={26} strokeWidth={2.4} />
                  </div>
                  <h3 className="text-xl font-bold text-[var(--color-navy)] mb-1">{cert.name}</h3>
                  <p className="text-xs text-[var(--color-navy)]/80 mb-5">
                    {cert.issuer}
                  </p>
                  <div className="relative w-full aspect-[4/3] rounded-xl overflow-hidden border border-[var(--color-rule)] bg-white">
                    <img
                      src={cert.image}
                    ...
                    <span className="absolute bottom-3 right-3 text-[10px] text-[var(--color-navy)]/85 bg-[var(--color-tint)]/70 px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
                      Hover to enlarge
                    </span>
                  </div>
                  <p className="mt-5 text-sm text-[var(--color-navy)]/85 leading-relaxed">{cert.blurb}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      >
        ...
        {certifications.map((cert) => (
          ...
          >
            ...
            <div className="text-center mt-4 text-[var(--color-navy)] text-sm">
